
from common_messages import invalid_number_of_params, not_registered_response
from helpers import can_be_int, valid_params_ignore_whitespace
from rewards import change_tokens
from safe_send import safe_send
from user.user import get_user_tokens, user_exists
from datetime import datetime, timezone
import math
import constants

async def donate_handler(db, message):
    
    valid_params, params = valid_params_ignore_whitespace(message, 3)
    if not valid_params:
        await invalid_number_of_params(message)
        return
    
    user = user_exists(db, message.author.id)
    if not user:
        await not_registered_response(message)
        return
    
    num_tokens_to_give = params[2]
    if not can_be_int(num_tokens_to_give):
        await safe_send(message.channel, 'Please enter a number for how many tokens to donate.')
        return
    
    int_tokens = int(num_tokens_to_give)
    if int_tokens <= 0:
        await safe_send(message.channel, "What? Umm, no you can't do that.")
        return

    user_tokens = get_user_tokens(user)
    if user_tokens < int_tokens:
        await safe_send(message.channel, 'You do not have enough tokens for this donation.')
        return
    
    account_age = datetime.now(timezone.utc) - message.author.created_at
    account_days = account_age.days
    if account_days < 30:
        account_days = int(math.floor(account_days))
        await safe_send(message.channel, 'Your Discord account is **'+str(account_days)+' days** old. To prevent alt account spam, only accounts 30 days or older can use the donate command.')
        return

    mentions = message.mentions
    if len(mentions) != 1:
        await safe_send(message.channel, 'Please mention 1 player to donate to.')
        return
    
    mentioned_user = mentions[0]
    donate_to_user = user_exists(db, mentioned_user.id)
    if not donate_to_user:
        await safe_send(message.channel, 'That user is not registered yet.')
        return
    
    if donate_to_user == user:
        await safe_send(message.channel, "You can't donate to yourself...")
        return
    
    if donate_to_user['discord_id'] == constants.BOT_ID:
        await safe_send(message.channel, "Thank you! But I don't need any tokens!")
        return
    
    await change_tokens(db, user, -1*int_tokens, 'donate')
    await change_tokens(db, donate_to_user, int_tokens, 'donate')
    await safe_send(message.channel, 'Donation successful!')
