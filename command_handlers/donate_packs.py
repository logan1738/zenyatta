...
async def donate_packs(db, message):
    ...
    if not can_be_int(num_packs_to_give):
        await safe_send(message.channel, 'Please enter a number for how many packs to donate. Has to be a whole number.')
        return
    ...
    if user_packs < int_packs:
        await safe_send(message.channel, 'You do not have enough packs for this donation.')
        return
    ...
    if donate_to_user['discord_id'] == constants.BOT_ID:
        await safe_send(message.channel, "Thank you! But I don't need any packs!")
        return
