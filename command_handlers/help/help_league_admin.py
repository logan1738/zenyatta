...
async def help_league_admin_handler(message):
    ...
    safe_add_field(help_embed, '!cancelinvite @User', "Admin command to cancel a user's pending invite to your team.", False)
    ...
    safe_add_field(help_embed, '!kick @Player', 'Admin command to kick a player from your league team.', False)
    ...
    safe_add_field(help_embed, '!order [spot 1] [spot 2]', 'Admin command to swap the positions of two players in the teams info page.', False)
