...
async def remove_team_admin_handler(db, message, client, context):
    ...
    if len(message.mentions) < 1:
        await safe_send(message.channel, 'Please mention the user to remove their admin.')
        return
    ...
    if not user_admin_on_team(mentioned_member.id, team_object):
        await safe_send(message.channel, 'That user is not an admin on your team.')
        return
    ...
    await remove_member_admin_role(mentioned_member, context, client)
